...
toInt :: (Num a)
      => Index -- ^ The considered index.
      -> a     -- ^ The corresponding integer.
...
isClosed :: Lambda -- ^ The considered lambda term.
         -> Bool   -- ^ True iff the lambda term is closed.
...
maxIndex :: (Num a, Ord a) 
         ...
         -> a      -- ^ The integer value of the maximal index.
